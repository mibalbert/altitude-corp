...
const ContactUs = ({ data, isEditable }) => {
  const contactUsTitle = data.find((el) => el.compName === "contact-title");
  const contactUsSubtitle1 = data.find(
    (el) => el.compName === "contact-subtitle-1"
  );
  const contactUsSubtitle2 = data.find(
    (el) => el.compName === "contact-subtitle-2"
  );
  const contactUsFormTitle = data.find(
    (el) => el.compName === "contact-form-title"
  );
  const contactUsFormSubtitle = data.find(
    (el) => el.compName === "contact-form-subtitle"
  );
